...
/* ----------------------------------------------------------------------------
 * LEGACY COMPAT SHIMS
 * @deprecated Ces exports existent pour ne pas casser les ~11 écrans pendant
 * la migration bloc par bloc. Cleanup final dans le BLOC 12.
 * ---------------------------------------------------------------------------- */
...
export const Colors = {
  ...
  dangerLight: COLORS.dangerMuted,
  shimmer: COLORS.surfaceAlt,
} as const;
...
// Ancien Typography (poids RN bruts sans Google Fonts)
export const Typography = {
...
// Anciennes occasions (préservé pour compat — mêmes clés, anciennes couleurs)
export const Occasions: Record<string, { label: string; emoji: string; color: string }> = {
  Anniversaire: { label: 'Anniversaire', emoji: '🎂', color: OCCASIONS.Anniversaire.color },
  Noël:         { label: 'Noël',         emoji: '🎄', color: OCCASIONS.Noël.color },
  Naissance:    { label: 'Naissance',    emoji: '👶', color: OCCASIONS.Naissance.color },
  Mariage:      { label: 'Mariage',      emoji: '💍', color: OCCASIONS.Mariage.color },
  Autre:        { label: 'Autre',        emoji: '🎁', color: OCCASIONS.Autre.color },
};
